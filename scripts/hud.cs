...
function serverCmdSetClientFav(%client, %text)
{
   if ( getWord( getField( %text, 0 ), 0 ) $= armor )
   {
    ...
    if (%client.pdepIndex > 0)
		%oldpDep = %client.favorites[getField(%client.pdepIndex,0)];
