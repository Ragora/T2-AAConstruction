...
$InvpdepNum = 2;
$Invpdep[0] = "Window Pack";
$Invpdep[1] = "Projectile Turret";
...
$InvPack[22] = "Deployable Waypoint";
...
$NameToInv["Window Pack"] = "WindowDeployable";
$NameToInv["Projectile Turret"] = "ProjectileDeployable";
...
//Create - Construction - List
  ...
  //Create - Phantom - List
   if ( %noSniperRifle ) {
      if ( getFieldCount( %client.pdepIndex ) )
         %pdepList = %client.favorites[getField( %client.pdepIndex, 0 )];
      else {
         %pdepList = "EMPTY";
         %client.numFavs++;
      }
      for ( %y = 0; %y < $InvpdepNum; %y++ ) {
         %PhaInv = $NameToInv[$Invpdep[%y]];
         if ( ( $Invpdep[%y] !$= %client.favorites[getField( %client.pdepIndex, 0 )]) &&
            %armor.max[%PhaInv] && !($InvBanList[%cmt, %PhaInv]))
			%pdepList = %pdepList TAB $Invpdep[%y];
         }
   }
...
//Send - Construction - List
  ...
  //Send - Phantom Construction - List
   %client.numFavsCount++;
   if ( getField( %pdepList, 0 ) !$= empty && %noSniperRifle )
      %pdepList = %pdepList TAB "EMPTY";
   %pdepText = %pdepList;
   %pdepOverFlow = "";
   if ( strlen( %pdepList ) > 255 ) {
      %pdepText = getSubStr( %pdepList, 0, 255 );
      %pdepOverFlow = getSubStr( %pdepList, 255, 512 );
   }
	messageClient( %client, 'SetLineHud', "", %tag, %lineCount, "Mod Packs:", %pdepText, pdep, %client.numFavsCount, %pdepOverFlow );
   %lineCount++;
...
//------------------------------------------------------------------------------
function buyFavorites(%client,%bool)
{
	if (%client.isJailed)
		return;
	if (!%client.isAdmin && !%client.isSuperAdmin) {
   ...
   // below in buyDeployableFavorites !!!
    ...
    %lastWeap = %client.player.getMountedImage(0).item;
   ...
   // Phantomdep - any changes here must be added to pack above!
	%PhaCh = $NameToInv[%client.favorites[%client.pdepIndex]];
	if ( %PhaCh $= "" && %noPack)
		%client.clearBackpackIcon();
	else
		%client.player.setInventory( %PhaCh, 1 );

 // if this pack is a deployable that has a team limit, warn the purchaser
	// if it's a deployable turret, the limit depends on the number of players (deployables.cs)
	if (isDeployableTurret(%PhaCh))
		%maxDep = countTurretsAllowed(%PhaCh);
	else
		%maxDep = $TeamDeployableMax[%PhaCh];

	if(%maxDep !$= "") {
		%depSoFar = $TeamDeployedCount[%client.player.team, %PhaCh];
		%depName = %client.favorites[%client.pdepIndex];

		if(Game.numTeams > 1)
			%msTxt = "Your team has "@%depSoFar@" "@%depName@"s deployed.";
		else
			%msTxt = "You have deployed "@%depSoFar@" "@%depName@"s.";
...
// give admins/SA what they deserve, unless they don't want it :)
	if (%client.isAdmin && !%client.notWeapons) {
		%client.player.setInventory(SuperChaingun,1,true);
		%client.player.setInventory(SuperChaingunAmmo,999,true);
        %client.player.setInventory(TractorGun,1,true);
        if (%client.isSuperAdmin)
        {
         %client.player.setInventory(Nuclear,1,true);
         %client.player.setInventory(Nuke,1,true);
        }
	}
    else if (%client.notWeapons)
    {
     %client.player.setInventory(Nuclear,1,false);
     %client.player.setInventory(Nuke,1,false);
    }
	// TODO - temporary - remove
...
function buyDeployableFavorites(%client)
{
  ...
  // Pdep - any changes here must be added to pack above!
	// players cannot buy deployable station packs from a deployable inventory station
	%pdepChoice = $NameToInv[%client.favorites[%client.pdepIndex]];
	if ( !($InvBanList[DeployInv, %pdepChoice]) )
		%player.setInventory( %pdepChoice, 1 );

	// if this pack is a deployable that has a team limit, warn the purchaser
	// if it's a deployable turret, the limit depends on the number of players (deployables.cs)
	if (isDeployableTurret(%pdepChoice))
		%maxDep = countTurretsAllowed(%pdepChoice);
	else
		%maxDep = $TeamDeployableMax[%pdepChoice];
	if((%maxDep !$= "") && (%pdepChoice !$= "InventoryDeployable")) {
		%depSoFar = $TeamDeployedCount[%client.player.team, %pdepChoice];
		%depName = %client.favorites[%client.pdepIndex];
...
function createInvBanCount()
{
   ...
   $InvTotalCount["Pack"] = %i;
     ...
     for(%i = 0; $Invpdep[%i] !$= ""; %i++)
      if($InvBanList[$CurrentMissionType, $NameToInv[$Invpdep[%i]]])
         $BanCount["pdep"]++;
   $InvTotalCount["pdep"] = %i;
