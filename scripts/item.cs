...
function cyclePackSetting(%plyr,%val) {
	...
	if (%plyr.hasSpine) {
        ...
        else if (%plyr.hasWindow) {
		%plyr.packSet = %plyr.packSet + %val;
		if (%plyr.packSet > $packSettings["window"])
			%plyr.packSet = 0;
		if (%plyr.packSet < 0)
			%plyr.packSet = $packSettings["window"];
		%line = $packSetting["window",%plyr.packSet];
		bottomPrint(%plyr.client,"Window set to" SPC getWords(%line,0,getWordCount(%line)),2,1);
		%changed = true;
	}
   else if (%plyr.hasProjectile) {
		%plyr.packSet = %plyr.packSet + %val;
		if (%plyr.packSet > $packSettings["projectileTurret"])
			%plyr.packSet = 0;
		if (%plyr.packSet < 0)
			%plyr.packSet = $packSettings["projectileTurret"];
		%line = $packSetting["projectileTurret",%plyr.packSet];
		bottomPrint(%plyr.client,"Projectile set to:" SPC getWords(%line,0,getWordCount(%line)),2,1);
		%changed = true;
	}
