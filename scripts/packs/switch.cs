...
function DeployedSwitch::onCollision(%data,%obj,%col) {
    if (%obj.set == 3 || %obj.set == 4)
    {
     toggleSwitch(%obj,-1,%col);
     if (%col.getClassName() $= "item" && %obj.set == 4)
     %col.delete();
     return;
    }
    else
...
function toggleSwitch(%obj,%state,%col,%delayed) {
	...
	if (%col == 0 || %col $= "")
		%force = true;
	if (!%force && %col.getClassName() !$= "item") { //The item check is here because the ifs in here are for players, causing console spam
    ...
    if (%col.getClassName() $= "Player" && (%obj.set == 3 || %obj.set == 4))
    {
     messageClient(%col.client,'msgClient',"\c3This is an item toggled switch.");
     return;
    }
