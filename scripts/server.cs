...
$ModVersion = "v0.10 Development";
$ModCredits = "Exerpt, Dark Dragon DX";
...
function CreateServer(%mission, %missionType)
{
   ...
   exec("scripts/modScripts/init.cs"); //Load custom modScripts
	if (!$Host::MTC::NoLoad)
...
function GameConnection::onConnect( %client, %name, %raceGender, %skin, %voice, %voicePitch )
{
   ...
   constructionClientJoined(%client);
...
function GameConnection::onDrop(%client, %reason)
{
   ...
   if ( $CurrentMissionType $= "SinglePlayer" )
    ...
    %logname = stripChars(getTaggedString(%client.name),"\x10\x11\c8\c7\c6");
	%logExport = formatTimeString(yy) @ "/" @ formatTimeString(mm) @ "/" @ formatTimeString(dd);
	%logExport = %logExport SPC formatTimeString(h) @ ":" @ formatTimeString(n) @ "." @ formatTimeString(s) SPC formatTimeString(a);
	...
	{ 
		...
		{ 
			...
			{ 
				...
				if(%admin.isAdmin && $PlayingOnline)
...
function GameConnection::startMission(%this)
{
   ...
   %this.currentPhase = 0;
   schedule(1000, 0, "debriefLoad", %this);
...
function limitObjectVelocity(%obj) {
	...
	if (!$Host::AllowUnderground) {
		...
		if (%obj.fogKilled)
			return;
		%pos = %obj.getPosition();
		%terrain = getWord(getTerrainHeight(%pos),2);
